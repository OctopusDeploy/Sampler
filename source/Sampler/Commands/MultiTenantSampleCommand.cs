...
namespace Octopus.Sampler.Commands
{
    ...
    public class MultiTenantSampleCommand : ApiCommand
    {
        ...
        public MultiTenantSampleCommand(IOctopusRepositoryFactory octopusRepositoryFactory)
        ...
        protected override void Execute()
        {
            ...
            var allEnvironments = new[] {"MT Dev", "MT Test", "MT Beta", "MT Staging", "MT Production"}.Select(name => Repository.Environments.CreateOrModify(name, LipsumRobinsonoKruso.GenerateLipsum(1)).Instance).ToArray();
            var normalLifecycle = Repository.Lifecycles.CreateOrModify("MT Normal Lifecycle", "The normal lifecycle for the multi-tenant deployments sample").AsSimplePromotionLifecycle(allEnvironments.Where(e => e.Name != "MT Beta").ToArray()).Save().Instance;
            var betaLifecycle = Repository.Lifecycles.CreateOrModify("MT Beta Lifecycle", "The beta lifecycle for the multi-tenant deployments sample").AsSimplePromotionLifecycle(allEnvironments.Take(3).ToArray()).Save().Instance;
            var projectGroup = Repository.ProjectGroups.CreateOrModify("Multi-tenancy sample").Instance;
            ...
            var tagSetTenantType = Repository.TagSets.CreateOrModify("Tenant type", "Allows you to categorize tenants")
                ...
                .Save().Instance;
            var tagSetImportance = Repository.TagSets.CreateOrModify("Tenant importance", "Allows you to have different customers that we should pay more or less attention to")
                ...
                .Save().Instance;
            var tagSetRing = Repository.TagSets.CreateOrModify("Upgrade ring", "What kind of upgrade stability to these customers want")
                ...
                .Save().Instance;
            var tagSetEarlyAccessProgram = Repository.TagSets.CreateOrModify("Early access program", "Provides tenants with access to certain early access programs.")
                .AddOrUpdateTag("2.x Beta", "These customers are part of our 2.x beta program", TagResource.StandardColor.LightPurple)
                .Save().Instance;
            var tagSetHosting = Repository.TagSets.CreateOrModify("Hosting", "Allows you to define where the tenant software should be hosted")
            ...
            var allTags = new TagResource[0]
                ...
                .Concat(tagSetEarlyAccessProgram.Tags)
                .ToArray();
            ...
            foreach (var internalSharedHostTag in tagSetHosting.Tags.Where(t => t.Name.StartsWith("Internal-Shared")))
            {
            ...
            foreach (var sharedHostTag in tagSetHosting.Tags.Where(t => t.Name.StartsWith("Shared-Farm")))
            {
                var sharedHosts = Enumerable.Range(0, 4).Select(i => Repository.Machines.CreateOrModify(
            ...
            var envVarEditor = Repository.LibraryVariableSets.CreateOrModify("Environment variables", "The environment details we require for all projects");
            foreach (var e in allEnvironments)
            {
                envVarEditor.Variables.AddOrUpdateVariableValue("Environment.Alias", e.Name.Replace("MT ", "").ToLowerInvariant(), new ScopeSpecification {{ScopeField.Environment, e.Id}});
            }
            envVarEditor.Save();

            var stdTenantVarEditor = Repository.LibraryVariableSets.CreateOrModify("Standard tenant details", "The standard details we require for all tenants");
            ...
            var testers = GetSampleTenantData("Tester", NumberOfTesters)
                .Select(x =>
                {
                    ...
                    FillOutTenantVariablesByConvention(tenantEditor, projects, allEnvironments, libraryVariableSets);
            ...
            var customers = GetSampleTenantData("Customer", NumberOfCustomers)
                .Select(x =>
                {
